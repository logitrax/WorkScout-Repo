...
if(!class_exists("b372b0Base")) {
	class b372b0Base {
    	public $key = "72E4295355727CA8";
    	private $product_id = "1";
    	private $product_base = "workscout";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_workscout_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=workscout_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		private function processs_response($response){
			...
			if ( ! empty( $response ) ) {
				...
				if ( is_object( $response ) ) {
					...
					add_filter('workscout_license_check', '__return_true');
// 					$response=new stdClass();
// 					$response->status = false;
// 					$response->msg    =
// 					"<strong>Response Error</strong>. <br> Your hosting blocks outgoing requests to our server and your license key can't be validated.  <br>Theme requires manual activation. In order to do that  
// please <br> <a style=\"background:#2271b1; margin:10px 0px; border:0px;\" target=\"_blank\" class=\"button button-primary\"href=\"https://workscout.in/workscout-activator.zip\">Download and Install Activation Plugin</a>.";
// 					if(!empty($bkjson)){
...
// 					$response->data = NULL;
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			$manual_activation = apply_filters('workscout_license_check', false);
			if ($manual_activation) {
				$get_saved_key = get_option('WorkScout_lic_Key');
				// if empty generate some license key
				if (empty($get_saved_key)) {
					$get_saved_key =  strtoupper(uniqid());
					
				} 
				$responseObj = (object) ['is_valid' => '1', 'expire_date' => '2025-01-01', 'support_end' => '2026-01-01', 'license_title' => 'Single License Manual Activation', 'license_key' => $get_saved_key, 'msg' => 'msg'];
                    ...
                    if ( ! empty( $response->status ) ) {
                        if ( ! empty( $response->data ) ) {
                            ...
                            if ( $licenseObj->is_valid ) {
                                ...
                                $this->SaveWPResponse( $responseObj );
                                unset( $responseObj->next_request );
								update_option('worksout_license_key_activated', 'yes');
                                return true;
                            } else {
