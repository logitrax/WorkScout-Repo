...
================================================== -->
			...
			<div class="container <?php echo esc_attr($layout); ?>">
				...
				<?php if (job_manager_user_can_view_job_listing($post->ID)) : ?>
								...
								<div class="content">
									...
									if ($private_messages) : ?>
										<?php if (is_user_logged_in()) : ?>
											<a href="#small-dialog" class="send-message-to-owner button popup-with-zoom-anim"><?php echo workscout_get_icon('email'); ?> <?php esc_html_e('Send Message', 'workscout'); ?></a>
										<?php else : ?>
											<?php
											$popup_login = get_option('workscout_popup_login');
											if ($popup_login == 'ajax') { ?>
												<a href="#login-dialog" class="send-message-to-owner button popup-with-zoom-anim"><?php echo workscout_get_icon('email'); ?> <?php esc_html_e('Login to Send Message', 'workscout'); ?></a>
											<?php } else {
												$login_page = get_option('workscout_profile_page'); ?>
												<a href="<?php echo esc_url(get_permalink($login_page)); ?>" class="send-message-to-owner button"><?php echo workscout_get_icon('email'); ?> <?php esc_html_e('Login to Send Message', 'workscout'); ?></a>
											<?php } ?>
								...
								<?php else : ?>
									...
									?>
										<ul class="share-post">
											...
											<?php if (in_array("twitter", $share_options)) { ?><li><?php echo '<a target="_blank" class="x-share" href="https://twitter.com/share?url=' . $url . '&amp;text=' . esc_attr($summary) . '" title="' . __('Share on X', 'workscout') . '"><svg xmlns="http://www.w3.org/2000/svg" width="21.413" height="20.879" viewBox="0 0 21.413 20.879"><path id="_11053970_x_logo_twitter_new_brand_icon" data-name="11053970_x_logo_twitter_new_brand_icon" d="M13.131,9.63,20.718,1h-1.8L12.329,8.492,7.069,1H1L8.957,12.33,1,21.379H2.8l6.956-7.913,5.557,7.913h6.069Zm-2.463,2.8L9.86,11.3,3.446,2.326H6.208l5.178,7.245.8,1.129,6.73,9.417H16.158Z" transform="translate(-0.447 -0.75)" fill="#fff" stroke="#fff" stroke-width="0.5"/></svg>' .  __('Share', 'workscout') . '</a>'; ?></li><?php } ?>
					...
					<div class="five columns" id="job-details">
						...
						<div class="widget">
							...
							<?php $overview_elements = Kirki::get_option('workscout', 'pp_job_overview', array('id', 'date_posted', 'expiration_date', 'application_deadline', 'location', 'job_title', 'hours', 'rate', 'salary', 'wpjm_salary'));  ?>
							<div class="job-overview">
								<?php do_action('single_job_listing_meta_before'); ?>
								<ul>
									<?php do_action('single_job_listing_meta_start'); ?>
									<?php if (in_array("id", $overview_elements)) : ?>
										<li>
											<i class="fa fa-address-card"></i>
											<div>
												<strong><?php esc_html_e('ID', 'workscout'); ?>:</strong>
									...
									<?php if (in_array("date_posted", $overview_elements)) : ?>
										<li>
											<?php echo workscout_get_icon('date'); ?>
											<div>
									...
									<?php if (in_array("expiration_date", $overview_elements)) : ?>
										...
										if (empty($hide_expiration)) {
											if (!empty($expired_date)) { ?>
												<li>
													<?php echo workscout_get_icon('expiration'); ?>
													<div>
									...
									<?php if (in_array("application_deadline", $overview_elements)) : ?>
										...
										if ($deadline = get_post_meta($post->ID, '_application_deadline', true)) {
											...
											echo '<li class="ws-application-deadline ' . ($expiring ? 'expiring' : '') . ' ' . ($expired ? 'expired' : '') . '">' . workscout_get_icon('expiration') . '
						<div>
							<strong>' . ($expired ? __('Closed', 'workscout') : __('Closes', 'workscout')) . ':</strong><span>' . date_i18n(get_option('date_format'), strtotime($deadline)) . '</span></div></li>';
									...
									<?php if (in_array("location", $overview_elements)) : ?>
										<li>
											...
											<?php echo workscout_get_icon('location'); ?>
											<div>
									...
									<?php if (in_array("job_title", $overview_elements)) : ?>
										<li>
											...
											<?php echo workscout_get_icon('title'); ?>
											<div>
									...
									<?php if (in_array("hours", $overview_elements)) : ?>
										...
										if ($hours) { ?>
											<li>
												<?php echo workscout_get_icon('hours'); ?>
												<div>
									...
									if (in_array("rate", $overview_elements)) : ?>
										...
										if ($rate_min) {
											...
											<li>
												...
												<?php echo workscout_get_icon('money'); ?>
												<div>
												...
												<div>
													...
													<span>
														...
														if (!empty($salary_max) && is_numeric($salary_max)  && $salary_max > $salary_min) {
															...
															echo esc_html(number_format_i18n($salary_max, $decimals));
															if ($currency_position == 'after') {
																echo get_workscout_currency_symbol();
															}
														} ?>
									...
									<?php endif; //overview elements 
									?>
									<?php if (in_array("wpjm_salary", $overview_elements)) :
										if (get_the_job_salary()) { ?>
											<li>
												<?php echo workscout_get_icon('money'); ?>
												<div>
													<strong><?php esc_html_e('Salary:', 'workscout'); ?></strong>
													<span><?php the_job_salary(); ?>
													</span>
												</div>
											</li>
									<?php }
