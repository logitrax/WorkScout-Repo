	...
	<div class="full-page-container with-map">
		...
		<div class="full-page-map-container">
			<?php $all_map = Kirki::get_option( 'workscout', 'pp_enable_all_jobs_map', 0 ); 
				if($all_map){ 
					echo do_shortcode('[workscout-map type="resume" class="jobs_page"]'); 
				} else { ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('eleven columns'); ?>>
		<div class="padding-right">
			...
			if(empty($search_in_sb)) {
				...
				if ( apply_filters( 'workscout_resumes_show_search_keywords', true ) ): ?>
				<form class="list-search"  method="GET" action="">
					<div class="search_keywords">
						<button><i class="fa fa-search"></i></button>
						<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php esc_attr_e('e.g. logo design, copywriting', 'workscout' ); ?>" value="<?php echo stripslashes(esc_attr( $keywords )); ?>" />
			...
			echo do_shortcode('[resumes  categories='.get_query_var('resume_category').' show_filters="false" show_pagination="true"]')?>
