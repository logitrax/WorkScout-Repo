...
/**
* Workscout_Freelancer_Paid_Listings_Subscriptions
*/
class Workscout_Freelancer_Paid_Listings_Subscriptions {
	...
	public function __construct() {
		...
		if ( class_exists( 'WC_Subscriptions_Synchroniser' ) && method_exists( 'WC_Subscriptions_Synchroniser', 'save_subscription_meta' ) ) {
			add_action( 'woocommerce_process_product_meta_task_package_subscription', 'WC_Subscriptions_Synchroniser::save_subscription_meta', 10 );
		}
	...
	private function get_listing_subscription_order_id( $listing_id ) {
		if ( 'listing' === get_post_type( $listing_id ) ) {
			...
			$user_package    = workscout_core_get_user_package( $user_package_id );
	...
	public function woocommerce_is_subscription( $is_subscription, $product_id ) {
		...
		if ( $product && $product->is_type( 'task_package_subscription') ) {
	...
	public function check_expired_listing( $post ) {
		...
		if ( 'listing' === $post->post_type ) {
			...
			if ( $package_product_id ) {
				...
				//if($subscription_type == 'task_package_subscription') {

					$new_count = $wpdb->get_var( $wpdb->prepare( "SELECT package_count FROM {$wpdb->prefix}workscout_core_user_packages WHERE id = %d;", $package_id ) );
					$new_count --;
	...
	if ( $id > 0 ) {
		...
		if ( 'listing' === $post_type) {
			...
			if ( $package_product_id ) {
				...
				$package  = $wpdb->get_row( $wpdb->prepare( "SELECT package_count, package_limit FROM {$wpdb->prefix}workscout_core_user_packages WHERE id = %d;", $package_id ) );
...
public function subscription_ended( $subscription, $paused = false ) {
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}workscout_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) );
		...
		if ( $user_package ) {
			...
			$listing_ids = workscout_core_get_listings_for_package( $user_package->id );
	...
	delete_post_meta(  $subscription->get_id(), 'workscout_core_subscription_packages_processed' );
}
...
public function subscription_activated( $subscription ) {
	...
	if ( get_post_meta( $subscription->get_id(), 'workscout_core_subscription_packages_processed', true ) ) {
		return;
	}
	...
	$parent_id         = ! empty( $parent ) ?  $parent->get_id() : null;
	$legacy_id         = isset( $parent_id ) ? $parent_id : $subscription->get_id();
	$wpdb->delete( "{$wpdb->prefix}workscout_core_user_packages", array(
		'order_id' => $legacy_id,
	) );
	$wpdb->delete( "{$wpdb->prefix}workscout_core_user_packages", array(
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		if ( $product->is_type( array( 'task_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
			
			// Give packages to user
			for ( $i = 0; $i < $item['qty']; $i ++ ) {
				$user_package_id = workscout_core_give_user_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
			...
			foreach ( $listing_ids as $listing_id ) {
				if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired','publish' ) ) ) {
					workscout_core_approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
	...
	update_post_meta( $subscription->get_id(), 'workscout_core_subscription_packages_processed', true );
}
...
public function subscription_renewed( $subscription ) {
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		if ( $user_package_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}workscout_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) ) ) {
			foreach ( $user_package_ids as $user_package_id ) {
				$package = workscout_core_get_user_package( $user_package_id );

				if ( $listing_ids = workscout_core_get_listings_for_package( $user_package_id ) ) {
					foreach ( $listing_ids as $listing_id ) {
...
public function switch_package( $user_id, $new_subscription, $old_subscription ) {
	...
	$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}workscout_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $old_subscription->id, $legacy_id, $old_subscription->product_id ) );

	if ( $user_package ) {
		// If invalid, abort
		if ( ! $new_subscription->product->is_type( array( 'task_package_subscription') ) ) {
			return false;
		}

		// Give new package to user
		$switching_to_package_id = workscout_core_give_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
		...
		if ( $new_subscription->product instanceof WC_Product_Property_Package_Subscription ) {
...
Workscout_Freelancer_Paid_Listings_Subscriptions::get_instance();