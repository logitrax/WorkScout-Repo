...
if (isset($_REQUEST['stripe-express-setup']) && $_REQUEST['stripe-express-setup'] == 'yes') {
	try {
		...
		if (!class_exists('\Stripe\Stripe')) {
			require_once(WORKSCOUT_CORE_PLUGIN_DIR . '/vendor/stripe/stripe-php/init.php');
		}
		...
		if ($account->charges_enabled) {
			...
			update_user_meta($user_id, 'workscout_core_payment_type', 'stripe');

			// Show success message
?><div class="notification success closeable">
				<p><?php esc_html_e('Your Stripe account has been successfully connected!', 'workscout'); ?></p>
				<a class="close"></a>
			</div><?php
				} else {
					?><div class="notification warning closeable">
				<p><?php esc_html_e('Your Stripe account is connected but not yet ready to accept payments. Please complete your account setup.', 'workscout'); ?></p>
				<a class="close"></a>
			</div><?php
				}
			} catch (\Throwable $e) {
				error_log('Stripe account retrieval error: ' . $e->getMessage());
					?><div class="notification error closeable">
			<p><?php echo esc_html(sprintf(__('Error connecting to Stripe: %s', 'workscout'), $e->getMessage())); ?></p>
			<a class="close"></a>
		</div><?php
			}
		}
		if (get_option('workscout_stripe_connect_account_type') == 'standard') {
			if (isset($_GET['code'])) {
				...
				$headers = array(
					'User-Agent'    => 'workscout Stripe Split Pay',
...
<div class="row" id="waller-row" data-numberFormat=<?php if (wc_get_price_decimal_separator() == ',') {
	...
	<div class="col-lg-4 col-md-6">
		<div class="dashboard-stat color-1">
			<div class="dashboard-stat-content wallet-totals">
				<h4><?php echo wc_price($balance, array('currency' => ' ', 'decimal_separator' => '.')); ?></h4> <span><?php esc_html_e('Withdrawable Balance', 'workscout_core') ?> <strong class="wallet-currency"><?php echo $currency_symbol; ?></strong></span>
			</div>
	...
	<div class="col-lg-4 col-md-6">
		<div class="dashboard-stat color-3">
			<div class="dashboard-stat-content wallet-totals">
				<h4><?php echo wc_price($data->earnings_total, array('currency' => ' ', 'decimal_separator' => '.')); ?></h4> <span><?php esc_html_e('Total Earnings', 'workscout_core'); ?> <strong class="wallet-currency"><?php echo $currency_symbol; ?></strong></span>
			</div>
	...
	<div class="col-lg-4 col-md-6">
		<div class="dashboard-stat color-2">
			<div class="dashboard-stat-content">
				<h4><?php echo $data->total_orders; ?></h4> <span><?php esc_html_e('Total Orders', 'workscout_core'); ?></span>
			</div>
...
<div class="row">
	<div class="col-lg-6">
		<div class="dashboard-box invoices with-icons margin-top-20">
			<div class="headline">

				<?php
				$workscout_commission_rate = get_user_meta($user_id, 'workscout_commission_rate', true);

				// if it's string "" or empty
				if (empty($workscout_commission_rate)) {

					$workscout_commission_rate = get_option('workscout_commission_rate', 10);
				}

				?>
				<h4><?php esc_html_e('Earnings', 'workscout_core') ?> <div class="comission-taken"><?php esc_html_e('Fee', 'workscout_core'); ?>: <strong><?php echo $workscout_commission_rate; ?>%</strong></div>
				</h4>
			</div>
			<div class="content">
				<?php if ($commissions) { ?>
					<ul class="dashboard-commission-list">
						<?php
						foreach ($commissions as $commission) {
						...
						?>
								<li class="commission-item commission-<?php echo $commission['status']; ?>"><i class="list-box-icon icon-material-outline-shopping-cart"></i>
									<strong><?php echo get_the_title($commission['project_id']) ?></strong>
									<?php if ($commission['status'] == 'paid') { ?> <span class="commission-tag-paid"><?php esc_html_e('Processed', 'workscout_core'); ?></span> <?php } ?>
									<ul class="commission-details">
										<li class="paid"><?php echo wc_price($total); ?></li>
										<li class="unpaid"><?php esc_html_e('Fee', 'workscout_core'); ?>: <?php echo wc_price($commission['amount']); ?></li>
										<li class="paid"><?php esc_html_e('Your Earning', 'workscout_core'); ?>: <span><?php echo wc_price($earning); ?></span></li>
										<li><?php esc_html_e('Order', 'workscout_core'); ?>: #<?php echo $commission['order_id']; ?></li>
										<li><?php esc_html_e('Date', 'workscout_core'); ?>: <?php echo date_i18n(get_option('date_format'), strtotime($commission['date']));  ?></li>
									</ul>
								</li>
						<?php endif;
						} ?>
				...
				<?php } else { ?>
					<ul class="dashboard-commission-list">
						<li class=" wallet-empty-list"><i class="list-box-icon icon-material-outline-shopping-cart"></i><?php esc_html_e('You don\'t have any earnings yet', 'workscout_core'); ?></li>
					</ul>
				<?php } ?>
			</div>
		</div>
	...
	<div class="col-lg-6">
		<?php
		$payouts_options = get_option('workscout_payout_options', array('bank', 'paypal'));

		$stripe_connect_status = get_option('workscout_stripe_connect_activation');
		if (!empty($payouts_options) || $stripe_connect_status == 'yes') {

			$payment_type =  (isset($current_user->workscout_core_payment_type)) ? $current_user->workscout_core_payment_type : ''; ?>


			<div class="dashboard-box payouts-method  with-icons margin-top-20">
				<div class="headline">
					<h4 style="position: relative;"><?php esc_html_e('Payout Methods', 'workscout_core') ?></h4>
				</div>
				<div class="content">
								...
								<form method="post" id="edit_user" action="<?php the_permalink(); ?>">
									<div class="payment payout-method-tabs">
										...
										if (!empty($payouts_options)) { ?>
											<?php if (in_array('paypal', $payouts_options)) { ?>
												...
												<div class="payment-tab <?php if ($payment_type == 'paypal') { ?>payment-tab-active <?php } ?>">
													<div class="payment-tab-trigger">
														...
														<label for="paypal"><?php esc_html_e('PayPal', 'workscout_core'); ?></label>
													</div>
														...
														<div class="row">
															<div class="col-md-12">
																<div class="card-label">
																	<label for="ppemail"><?php esc_html_e('PayPal Email', 'workscout_core'); ?></label>
																	<input id="ppemail" name="ppemail" value="<?php if (isset($current_user->workscout_core_ppemail)) {
																													echo $current_user->workscout_core_ppemail;
																												} ?>" type="email">
																</div>
															</div>
														</div>
											...
											<?php if (in_array('bank', $payouts_options)) { ?>
												<div class="payment-tab <?php if ($payment_type == 'banktransfer') { ?>payment-tab-active <?php } ?> ">
													<div class="payment-tab-trigger">
														...
														<label for="creditCart"><?php esc_html_e('Bank Transfer', 'workscout_core'); ?></label>
													</div>
													...
													<div class="payment-tab-content">
														<div class="row">
															...
															<div class="col-md-12">
																<div class="notice notification payout-method-notification"><strong><?php esc_html_e('Add following bank transfer details:', 'workscout_core'); ?></strong> <?php esc_html_e('account', 'workscout_core'); ?> <?php esc_html_e('holders name & address, account number, bank name, IBAN, BIC/SWIFT', 'workscout_core'); ?></div>
																<div class="card-label">
																	<label for="cvv"><?php esc_html_e('Bank Transfer Details', 'workscout_core'); ?></label>
																	<textarea id="cvv" name="bank_details" type="text"><?php if (isset($current_user->workscout_core_bank_details)) {
																															echo $current_user->workscout_core_bank_details;
																														} ?></textarea>
																</div>
															</div>
										...
										<?php if (get_option('workscout_stripe_connect_activation') == 'yes') { ?>
											...
											<div class="payment-tab <?php if (!in_array($payment_type, array('paypal', 'banktransfer'))) { ?>payment-tab-active <?php } ?> ">
												<div class="payment-tab-trigger">
													...
													<label for="stripe"><?php esc_html_e('Stripe Connect', 'workscout_core'); ?></label>
												</div>
														...
														<div class="col-md-12">
															<div class="card-label stripe-card-label">
																...
																if (get_user_meta($user_id, 'vendor_connected', true) == 1) { ?>
																	<label style="padding: 0; margin: 0;"><?php _e('You are connected with Stripe', 'workscout_core'); ?></label><br>
																	<a class="stripe-btn disconnect-stripe-button button" href="#"><?php _e('Disconnect Stripe Account', 'workscout_core'); ?></a>
																	<?php
																	...
																	if (get_option('workscout_stripe_connect_account_type') == 'standard') {
																		...
																		// Show OAuth link
																		$authorize_request_body = apply_filters('workscout_stripe_authorize_request_params', array(
																			'response_type' => 'code',
																			'scope' => 'read_write',
																			'client_id' => $client_id,
																		...
																		if (get_option('workscout_stripe_connect_account_type') == 'express') {
																	...
																	?><a href=<?php echo esc_url($url); ?> target="_self" class="conntect-w-stripe-btn"><?php _e('Connect with Stripe', 'workscout_core'); ?></a>
																		<?php  } else {
																		$account_id = get_user_meta($current_user->ID, 'workscout_stripe_express_account_id', true);
																		...
																		?>
																			<a href="<?php echo $account_url; ?>" target="_self" class="conntect-w-stripe-btn"><?php _e('Connect with Stripe', 'workscout_core'); ?></a>

																		<?php  } else { ?>
																			<a href="#" target="_self" class="workscout-create-stripe-express-link-account conntect-w-stripe-btn">
																				<i class="fa fa-circle-o-notch fa-spin" aria-hidden="true"></i><?php _e('Create Stripe Account', 'workscout_core'); ?>
																			</a>
																			<a style="display:none" href="#" target="_self" class="real-conntect-w-stripe-btn conntect-w-stripe-btn"><?php _e('Connect with Stripe', 'workscout_core'); ?></a>
																		<?php } ?>
																<?php }
																} ?>
									...
									<button class="button margin-top-15"><?php esc_html_e('Save', 'workscout_core') ?></button>
		...
		<?php if (get_user_meta($user_id, 'vendor_connected', true) == 1) { ?>
			...
			?>
					...
					<div class="dashboard-box invoices with-icons margin-top-20">
						<div class="headline">
							...
							<h4 style="position: relative;"><?php esc_html_e('Stripe Account Details', 'workscout_core') ?>
								...
								?>
									<br><a class="payout-method button" href="<?php echo $login_link['url']; ?>"><?php esc_html_e('Go to Stripe Dashboard', 'workscout_core') ?></a>
								<?php }
								if ($stripe_user_data['type'] == 'standard') { ?>
									<a class="payout-method button" href="https://dashboard.stripe.com/b/<?php echo $stripe_user_data['id']; ?>"><?php esc_html_e('Edit Account', 'workscout_core') ?></a>
								<?php } ?>
						...
						<div class="content">
							<ul class="dashboard-commission-list">
								<li><i class="list-box-icon icon-material-outline-monetization-on"></i>
									<strong><?php esc_html_e('Account Information', 'workscout_core') ?></strong>
									<ul>
										...
										<?php if ($stripe_user_data['payouts_enabled']) { ?>
											<li><?php esc_html_e('Payouts Enabled', 'workscout_core') ?></li>
										<?php } else { ?>
											<li><?php esc_html_e('Payouts Disabled', 'workscout_core') ?></li>
										<?php } ?>
										<li></li>
									</ul>
								</li>
								...
								try {
									...
									<li class="commission-item"><i class="list-box-icon icon-material-outline-monetization-on"></i>
										<strong><?php esc_html_e('Payments', 'workscout_core') ?></strong>
										<ul>
											<?php
											foreach ($balance['available'] as $key => $value) { ?>
												<li class="paid"><?php esc_html_e('Available Balance:', 'workscout_core') ?> <?php echo wc_price(($value->amount) / 100); ?> </li>

											<?php } ?>
											<?php
											foreach ($balance['pending'] as $key => $value) { ?>
												<li class="paid"><?php esc_html_e('Pending Balance:', 'workscout_core') ?> <?php echo wc_price(($value->amount) / 100); ?> </li>
					...
					<div class="dashboard-box invoices with-icons margin-top-20">
						<div class="headline">
							<h4 style="position: relative;"><?php esc_html_e('Stripe Account Details', 'workscout_core') ?></h4>
						</div>
						<div class="content">
							<ul class="dashboard-box-list">
								<li><i class="list-box-icon sl sl-icon-user-following"></i>
			...
			<div class="dashboard-box invoices with-icons margin-top-20">
				...
				<div class="headline">
					<h4 style="position: relative;"><?php esc_html_e('Payout History', 'workscout_core') ?></h4>
				</div>
				<?php if ($payouts) { ?>
					<ul class="dashboard-commission-list">
						<?php
						foreach ($payouts as $payout) {
							...
							if ($payout['payment_method'] === 'paypal') {
								$payment_method =  __('PayPal', 'workscout_core');
							} else if ($payout['payment_method'] === 'PayPal Payout') {
								$payment_method = __('PayPal Payout', 'workscout_core');
							} else {
								$payment_method = __('Bank Transfer', 'workscout_core');
							}


						?>
							<li><i class=" list-box-icon icon-material-outline-account-balance-wallet"></i>
								<strong><?php echo wc_price($payout['amount']) ?></strong>
								<ul>
									<li class="payment_method"><?php echo $payment_method; ?></li>
									<li><?php esc_html_e('Date', 'workscout_core') ?>: <?php echo date_i18n(get_option('date_format'), strtotime($payout['date']));  ?></li>
								</ul>
							</li>
				...
				<?php } else { ?>
					<ul class="dashboard-commission-list">
						<li class="wallet-empty-list"><i class="list-box-icon icon-material-outline-account-balance-wallet"></i> <?php esc_html_e('You don\'t have any payouts yet.', 'workscout_core') ?></li>
					</ul>
				<?php } ?>
			</div>
					</div>
