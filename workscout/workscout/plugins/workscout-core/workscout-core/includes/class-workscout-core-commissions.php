 ...
 */
class Workscout_Core_Commissions {
	...
	public function __construct() {
		...
		add_shortcode( 'workscout_wallet', array( $this, 'workscout_wallet' ) );
	...
	function delete_commission($order_id){
		if ( ! $order_id ) {
			...
			global $wpdb;
			$wpdb->delete(  $wpdb->prefix . "workscout_core_commissions", array( 'order_id' => $order_id ) );
	}
	...
	function register_commission($order_id){
		...
		$processed = $order->get_meta( '_workscout_commissions_processed', true );
		...
		$args['user_id'] = $order->get_meta('freelancer_id');
		$args['project_id'] = $order->get_meta('project_id');
		$args['milestone_id'] = $order->get_meta('milestone_id');
		

		$commission = get_user_meta($args['user_id'], 'workscout_commission_rate', true);
		if (empty($commission)) {
			$commission = get_option('workscout_commission_rate', 10);
		}
		...
		$connect_processed = $order->get_meta('workscout_stripe_connect_processed', true);
		...
		if($commission_id){
			$order->add_meta_data( '_workscout_commissions_id', $commission_id, true );
			$order->add_meta_data( '_workscout_commissions_processed', 'yes', true );
	...
	function insert_commission($args){
		...
		$wpdb->insert( $wpdb->prefix . "workscout_core_commissions", (array) $args );
	...
	function get_commissions($args){
		...
		global $wpdb;
			...
			$default_args = array(
				...
				'order'        => 'DESC',
				'fields'       => 'ids',
                'table'        => $wpdb->prefix . "workscout_core_commissions"
			);
			...
			if ( ! empty( $args['milestone_id'] ) ) {
				$where .= $wpdb->prepare( " AND c.milestone_id = %d", $args['milestone_id'] );
			}
	...
	function get_commission($commission_id){
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ". $wpdb->prefix . "workscout_core_commissions WHERE ID = %d", $commission_id ), ARRAY_A );
	}
	/**
	 * User wallet page shortcode
	 */
	public function workscout_wallet( $atts ) {

		if ( ! is_user_logged_in() ) {
			return __( 'You need to be signed in to access your wallet.', 'workscout_core' );
		}
...
if ( ! class_exists( 'Workscout_Balances_List_Table' ) ) {
    /**
     *
     *
     ...
     *
     */
    class Workscout_Balances_List_Table extends WP_List_Table {
    /** Class constructor */
        public function __construct() {

            parent::__construct( [
                'singular' => __( 'User Balance', 'workscout_core' ), // singular name of the listed records
                'plural'   => __( 'Users Balance', 'workscout_core' ), // plural name of the listed records
        ...
        public function get_columns() {
            $columns = array(
                    'user_id'   => __( 'User ID', 'workscout_core' ),
                    'user_name' => __( 'User Name', 'workscout_core' ),
                    'balance'   => __( 'Balance to pay', 'workscout_core' ),
                    'orders'    => __( 'Orders counter', 'workscout_core' ),
        ...
        * Define which columns are hidden
        ...
        private function table_data() {
            ...
            $commission = new Workscout_Core_Commissions;
        ...
        public function column_default( $item, $column_name )
        {
            switch( $column_name ) {
                ...
                case 'actions':
                $url = admin_url( 'admin.php?page=workscout_payouts');
                
                $payout_url = esc_url( add_query_arg( 'make_payout', $item['user_id'], $url ) );
               
                printf( '<a class="button-primary view" href="%1$s" data-tip="%2$s">%2$s</a>', $payout_url, __( 'Make Payout', 'workscout_core' ) );
                break;
        ...
        function no_items() {
            _e( 'No users found.','workscout_core' );
        }
