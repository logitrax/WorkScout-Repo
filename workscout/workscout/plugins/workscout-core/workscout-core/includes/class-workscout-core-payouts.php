...
class Workscout_Core_Payouts {
    ...
    public function add_menu_item() {
            ...
            $args = apply_filters( 'workscout_core_commissions_menu_items', array(
                    'page_title' => __( 'Commissions', 'workscout_core' ),
                    'menu_title' => __( 'Commissions', 'workscout_core' ),
                    'capability' => 'edit_products',
                    'menu_slug'  => 'workscout_payouts',
            ...
            add_submenu_page( 'workscout_payouts', 'Payouts History', 'Payouts History', 'edit_products', 'workscout_payouts_list', array( $this, 'payouts_details_page' ) );
    }
    ...
    public function make_payout(){
        ...
        if(isset($_POST['commission']) && !empty($_POST['commission'])){
            if(is_array($_POST['commission'])){
                foreach ($_POST['commission'] as $key => $value) {
                    $orders_list[] = $key;
                    $wpdb->update( $wpdb->prefix . "workscout_core_commissions", array('status'=>'paid'), array( 'id' => $key ) );
                    $order_id = $wpdb->get_var( $wpdb->prepare( 
                        '
                            SELECT order_id 
                            FROM '.$wpdb->prefix.'workscout_core_commissions
                            WHERE id = %s
                        ', 
                        $key
                    ) );
        ...
        $wpdb->insert( $wpdb->prefix . "workscout_core_commissions_payouts", $args );

        if($wpdb->insert_id){
            echo '<div class="updated"><p>'.esc_html__('Payout was created.','workscout_core').'</p></div>';
            $id = $wpdb->insert_id; 
            ...
            <div class="wrap">
                ...
                <ul>
                    <li><span><?php esc_html_e( 'Payment for', 'workscout_core' ); ?></span> <?php echo $user_data->display_name; ?></li>
                    <li><span><?php esc_html_e('Payment date','workscout_core'); ?>:</span> <?php echo date(get_option( 'date_format' ), strtotime($payout['date']));  ?></li>
                    <li><span><?php esc_html_e('Payment amount','workscout_core'); ?>:</span>  <?php echo wc_price($payout['amount']); ?></li>
                    <li><span><?php esc_html_e('Payment method','workscout_core'); ?>:</span> <?php echo $payment_method ; ?></li>
                    <li><span><?php esc_html_e('Payment details','workscout_core'); ?>:</span>  
                ...
                <?php 
                $commission_class = new Workscout_Core_Commissions;
                $commissions = array();
                ...
                <?php if($commissions) {?>
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        foreach ($commissions as $commission) { 
                            ...
                            ?>
                            <tr>
                                <td><?php echo get_the_title($commission['project_id']) ?></td>
    ...
    public function commissions_details_page() {
        ...
        if(isset($_POST['submit_new_payout'])){
            ...
            $payment_type = get_user_meta($user_id, 'workscout_core_payment_type', true);
            ...
            ?>
            <div class="wrap">
                <h2><?php esc_html_e( 'New Payment for', 'workscout_core' ); ?> <?php echo $user_data->display_name; ?></h2>
                <div class="payout-make-box">
                ...
                <form method="POST"  id="workscout-make-payout">
                    <input type="submit" name="submit_new_payout" id="submit" class="button button-primary" value="Make Payout">
                    <input type="hidden" name="action" value="workscout_make_payout" />
                    ...
                    <?php if($payment_method == 'paypal'): ?>
                        <input type="hidden" name="payment_details" value="<?php echo $user_data->workscout_core_ppemail ?>">
                    <?php endif; ?> 
                    <?php if($payment_method == 'banktransfer'): ?>
                        <input type="hidden" name="payment_details" value="<?php echo esc_attr($user_data->workscout_core_bank_details); ?>">
                    <?php endif; ?>
                    ...
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        foreach ($commissions as $commission) { 
                            ...
                            <tr>
                                ...
                                <?php if (workscout_is_payout_active()): ?>
                                    ...
                                    <?php else: ?>
                                        <td><em><?php _e('First, you need to cancel automatic commission that was previously sent by using PayPal Payout. Then you can send the commission manually.', 'workscout'); ?></em></td>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <td><input type="checkbox" checked="checked" name="commission[<?php echo $commission['id']; ?>]"></td>
                                <?php endif; ?>
                                ...
                                <td class="paid"> <span><?php echo wc_price($earning); ?></span></td>
                                <td>#<?php 
                                // get admin link to view order in WooCommerce -> Orders
                                $order_link = admin_url( 'post.php?post=' . $commission['order_id'] . '&action=edit' );
                                ?><a href="<?php echo $order_link; ?>" target="_blank"><?php
                                echo $commission['order_id']; ?> </a>
                                </td>
                                <td><?php echo date(get_option( 'date_format' ), strtotime($commission['date']));  ?></td>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                                <a class="button color cancel-payout-button" href="#" data-pp_item_id="<?php echo $commission['payout_item_id']; ?>" data-commission_id="<?php echo $commission['id']; ?>">
                                                    ...
                                                    <?php _e('Cancel PayOut', 'workscout'); ?>
                                                </a>
                                            <?php endif; ?>
                                        </td>
                                    <?php else: ?>
                ...
                <input type="submit" name="submit_new_payout" id="submit" class="button button-primary" value="Make Payout">
                    ...
                    <div class="loader-div workscout-hidden">
        ...
        <?php } 
            ...
            $balances_table = new Workscout_Balances_List_Table();
            ...
            <div class="wrap">
                ...
                <form id="commissions-filter" method="get">
                    ...
                    <?php // $commissions_table->add_search_box( __( 'Search commissions', 'workscout_core' ), 's' ); ?>
    ...
    public function payouts_details_page() {
        ...
        <?php } else {
            ...
            $payouts_table = new Workscout_Payouts_List_Table();
    ...
    function get_payout($id){
        ...
        global $wpdb;
        return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ". $wpdb->prefix . "workscout_core_commissions_payouts WHERE ID = %d", $id ), ARRAY_A );  
    }
    function get_payouts($args){
        ...
        global $wpdb;
            ...
            $default_args = array(
                ...
                'order'        => 'DESC',
                'fields'       => 'ids',
                'table'        => $wpdb->prefix . "workscout_core_commissions_payouts"
            );
...
if ( ! class_exists( 'Workscout_Payouts_List_Table' ) ) {
    /**
     *
     *
     ...
     *
     */
    class Workscout_Payouts_List_Table extends WP_List_Table {
    /** Class constructor */
        public function __construct() {

            parent::__construct( [
                'singular' => __( 'Payout', 'workscout-core' ), // singular name of the listed records
                'plural'   => __( 'Payouts', 'workscout-core' ), // plural name of the listed records
        ...
        public function get_columns() {
            $columns = array(
                    'id'             => __( 'ID', 'workscout_core' ),
                    'user_id'        => __( 'User', 'workscout_core' ),
                    //'status'         => __( 'Status', 'workscout_core' ),
                    'orders'         => __( 'Orders number', 'workscout_core' ),
                    'payment_method' => __( 'Payment method', 'workscout_core' ),
                    'amount'         => __( 'Amount', 'workscout_core' ),
                    'date'           => __( 'Date', 'workscout_core' ),
                    'actions'   => __( 'Actions', 'workscout_core' ),
        ...
        private function table_data() {
            ...
            $payouts_class = new Workscout_Core_Payouts;
        ...
        public function column_default( $item, $column_name ){
            switch( $column_name ) {
                ...
                case 'actions':
                $url = admin_url( 'admin.php?page=workscout_payouts_list');
                
                $payout_url = esc_url( add_query_arg( 'view_payout', $item['id'], $url ) );
               
                printf( '<a class="button-primary view" href="%1$s" data-tip="%2$s">%2$s</a>', $payout_url, __( 'View Details', 'workscout_core' ) );
                break;
                default:
        ...
        function no_items() {
            _e( 'No payouts set.','workscout_core' );
        }
