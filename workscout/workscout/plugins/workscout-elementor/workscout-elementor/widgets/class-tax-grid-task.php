 ...
 * @since 1.0.0
 */
class TaskTaxonomyGrid extends Widget_Base
{
	...
	{
		return 'workscout-taxonomy-grid-task';
	}
	...
	{
		return __('Taxonomy Grid - Tasks', 'workscout_elementor');
	}
	...
	protected function register_controls()
	{
		...
		$taxonomy_names = get_object_taxonomies('task', 'object');
		foreach ($taxonomy_names as $key => $value) {

			$this->add_control(
				$value->name . '_include',
				[
					'label' => __('Include tasks from ' . $value->label, 'workscout_elementor'),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
			...
			$this->add_control(
				$value->name . '_exclude',
				[
					'label' => __('Exclude tasks from ' . $value->label, 'workscout_elementor'),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
		...
		$this->add_control(
			'show_counter',
			[
				'label' => __('Show tasks listings counter', 'workscout_elementor'),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => __('Show', 'your-plugin'),
				'label_off' => __('Hide', 'your-plugin'),
				'return_value' => 'yes',
				...
				?>
					<a href="<?php echo get_term_link($term); ?>" class="new-category-box">
						...
						<?php if ($settings['show_counter'] == "yes") {
							$count = workscout_get_term_post_count('task_category', $term->term_id); ?> <div class="category-box-counter counter"><?php echo $count; ?></div><?php } ?>
...
<?php
	...
	{
		$taxonomies = get_object_taxonomies('task', 'objects');
