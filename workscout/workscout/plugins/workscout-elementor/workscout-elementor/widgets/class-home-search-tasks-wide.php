 ...
 * @since 1.0.0
 */
class HomeSearchTasksWide extends Widget_Base
{
	...
	{
		return 'workscout-homesearch-tasks-wide';
	}
	...
	{
		return __('Home Search Tasks Wide', 'workscout_elementor');
	}
	...
	protected function register_controls()
	{
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'type'    => \Elementor\Controls_Manager::TEXT,
				'default' => __('Task title, Skill, Industry', 'workscout_elementor'),
				'condition' => [
					'searchform' => 'keyword'
				],
			)
					...
					if(!empty($terms)) : ?>
								...
								<div class="highlighted-categories">
									...
									foreach ($terms as $value) {
										$term = get_term($value, 'task_category');
										if( $term && !is_wp_error( $term ) ) {
											$icon = get_term_meta($term->term_id,'icon',true); 
											$_icon_svg = get_term_meta($term->term_id,'_icon_svg',true);
											?>
											<!-- Box -->
											<a href="<?php echo get_term_link($term->slug, 'task_category'); ?>" class="highlighted-category">
